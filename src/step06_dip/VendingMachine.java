...
public class VendingMachine implements UserOperations, AdminOperations {
    private final Storable inventory; // 구체 클래스가 아닌 Storable 인터페이스에 의존
    private final Payable paymentProcessor; // 구체 클래스가 아닌 Payable 인터페이스에 의존
    private final Map<String, Item> items; // Drink 타입으로 변경

    // 생성자를 통해 모든 부품(의존성)을 외부에서 주입(Injection)받는다.
    public VendingMachine(List<Item> drinks, Storable inventory, Payable paymentProcessor) {
        // 외부에서 받은 Drink 리스트로 아이템 맵을 구성
        this.items = drinks.stream()
                .collect(Collectors.toMap(Item::getName, Function.identity()));

        // VendingMachine은 더 이상 객체를 직접 생성하지 않는다!
